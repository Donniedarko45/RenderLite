...
- **Nixpacks fails with `/nixpacks: no such file or directory` or `exec: "build": executable file not found`**: `ghcr.io/railwayapp/nixpacks:latest` is a Nix *base* image only (no CLI binary). Pull latest code and rebuild `worker`: the image installs the official `nixpacks` release tarball, and the Docker fallback downloads the same CLI inside `ghcr.io/railwayapp/nixpacks:ubuntu` before running `nixpacks build`. Optional: set `NIXPACKS_RELEASE` (same tag as Dockerfile `ARG`) if you pin a different version.
